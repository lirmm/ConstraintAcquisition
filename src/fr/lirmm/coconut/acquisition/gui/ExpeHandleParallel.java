...
public class ExpeHandleParallel extends ExpeHandle {
	...
	public ExpeHandleParallel(ACQ_Algorithm mode, DefaultExperience expe, int nbThread) {
		this(mode, expe, nbThread, null, null);
	}

	public ExpeHandleParallel(ACQ_Algorithm mode, DefaultExperience expe, int nbThread, String learned_csspath,
			String bias_csspath) {
	...
	protected void prepareExpe() {
		...
		ACQ_PACQ_Manager manager;
		switch (mode) {
		default:
			manager = new ACQ_PACQ_Manager(queries_mailbox);
		}

		manager.setBias(bias);
		manager.setLearnedNetwork(bias.getVars());
		manager.applyPartitioning(nbThread);
		ACQ_Network learnedNetwork = manager.getLearned_network();
					...
					Runnable runnable = new Runnable() {
						...
						public void run() {
							...
							for (int i = 0; i < nbThread; i++) {
								p = new ACQ_PACQ_Runnable(i, expe, bias, observedLearner, manager, queries_mailbox, stats);
								executor.execute(p);
							...
							ACQ_WS.Printstats(stats, nbThread, observedLearner, bias, expe, null);
						}
					};
