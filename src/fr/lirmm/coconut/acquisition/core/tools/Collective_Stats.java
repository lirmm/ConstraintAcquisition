...
public class Collective_Stats {
	...
	public double calculate_query_SD() {
		...
		int length = this.getStatManagers().size();

		for (int i = 0; i < this.getStatManagers().size(); i++) {
			sum += (this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery());

			/*System.out.println(this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery());*/
		}
		...
		for (int i = 0; i < this.getStatManagers().size(); i++) {
			standardDeviation += Math.pow((this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery()) - mean, 2);
		}
	...
	public double get_query_min() {
		double min = this.getStatManagers().get(0).getNbCompleteQuery()
				+ this.getStatManagers().get(0).getNbPartialQuery();

		for (int i = 1; i < this.getStatManagers().size(); i++) {
			if (min == 0 || ((this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery()) != 0
					&& min > (this.getStatManagers().get(i).getNbCompleteQuery()
							+ this.getStatManagers().get(i).getNbPartialQuery())))
				min = this.getStatManagers().get(i).getNbCompleteQuery()
						+ this.getStatManagers().get(i).getNbPartialQuery();
	...
	public double get_query_max() {
		double max = this.getStatManagers().get(0).getNbCompleteQuery()
				+ this.getStatManagers().get(0).getNbPartialQuery();

		for (int i = 1; i < this.getStatManagers().size(); i++) {
			if (max < (this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery()))
				max = this.getStatManagers().get(i).getNbCompleteQuery()
						+ this.getStatManagers().get(i).getNbPartialQuery();
	...
	public void Printstats(int nb_threads, ACQ_Learner learner, ACQ_Bias bias, DefaultExperience expe,
		...
		for (int i = 0; i < this.getChronos().size(); i++) {
			...
			double totalTime = 0;
			totalTime = (double) this.getChronos().get(i).getResult("total") / 1000.0;
			System.out.println("------Execution times------");

			for (String serieName : this.getChronos().get(i).getSerieNames()) {
				if (!"total".equals(serieName)) {
					double serieTime = (double) this.getChronos().get(i).getResult(serieName) / 1000.0;
					totaltime += serieTime;
	...
	public void ComputeGlobalStats(int nb_threads, ACQ_Learner learner, ACQ_Bias bias, DefaultExperience expe,
			int shared_queries) {
		double totaltime = 0.0;
		int totalqueries = 0;
		for (int i = 0; i < this.getChronos().size(); i++) {
			max_t = this.getTimeManagers().get(i).getMax();
			avg_t = this.getTimeManagers().get(i).getAverage();
			this.non_asked_queries.put(i, this.getStatManagers().get(i).getNon_asked_query());
			this.MQ_i.put(i, this.getStatManagers().get(i).getNbCompleteQuery());
			this.nb_queries_i.put(i, this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery());
			totalqueries += this.getStatManagers().get(i).getNbCompleteQuery()
					+ this.getStatManagers().get(i).getNbPartialQuery();
			this.nb_queries = totalqueries;
			this.avg_query_size.put(i, this.getStatManagers().get(i).getQuerySize());
			DecimalFormat df = new DecimalFormat("0.00E0");
			double totalTime = 0;
			try {
				totalTime = (double) this.getChronos().get(i).getResult("total") / 1000.0;
		...
		public void ComputeGlobalStats(int nb_threads, ACQ_Learner learner, ACQ_Bias bias, DefaultExperienceConacq expe,
				int shared_queries) {
			double totaltime = 0.0;
			int totalqueries = 0;
			for (int i = 0; i < this.getChronos().size(); i++) {
