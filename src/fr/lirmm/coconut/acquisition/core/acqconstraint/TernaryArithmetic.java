...
/**
 * Constraint between three variables. For example : X + Y <= Z or X - Y + Z =
 * 19
 * 
 */
public class TernaryArithmetic extends TernaryConstraint {
	...
	/**
	 * Returns a new BinaryArithmetic constraint which is the negation of this
	...
	public TernaryArithmetic getNegation() {
		...
		return new TernaryArithmetic("not_" + getName(), getScope().getFirst(), Operator.getOpposite(op1),
				getScope().getSecond(), Operator.getOpposite(op2), getScope().getThird(), cste);
	...
	public Constraint[] getChocoConstraints(Model model, IntVar... intVars) {
		...
		return new Constraint[] { model.arithm(intVars[this.getVariables()[0]], op1.toString(),
				intVars[this.getVariables()[1]], op2.toString(), intVars[this.getVariables()[2]]) };
	...
	public void toReifiedChoco(Model model, BoolVar b, IntVar... intVars) {
		...
		model.arithm(intVars[first], op1.toString(), intVars[second], op2.toString(), intVars[third]).reifyWith(b);
